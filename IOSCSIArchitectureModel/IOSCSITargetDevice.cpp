				...
				IOSCSIProtocolServices * provider )
{
	...
	__Require_noErr ( newDevice, ErrorExit );
	
	// Init the object.
	result = newDevice->init ( 0 );
	__Require ( result, ReleaseTargetExit );
	
	// Attach the object.
	result = newDevice->attach ( provider );
	__Require ( result, DetachTargetExit );
...
ReleaseTargetExit:
	...
	__Require_noErr_Quiet ( newDevice, ErrorExit );
	newDevice->release ( );
	newDevice = NULL;
...
IOSCSITargetDevice::SetNodeUniqueIdentifier ( OSObject * uniqueID )
{
	...
	__Require_noErr ( uniqueID, ErrorExit );
								 ...
								 void *				arg )
{
	...
	__Require_noErr ( fClients, ErrorExit );
    __Require_noErr ( OSDynamicCast ( IOSCSILogicalUnitNub, client ), ErrorExit );
...
IOSCSITargetDevice::handleIsOpen ( const IOService * client ) const
{
	...
	__Require_noErr ( fClients, CallSuperClassError );
	...
	switch ( type )
	{
		...
		case kSCSIPort_NotificationStatusChange:
		{
			...
			if ( ( fPathManager != NULL ) && ( path != NULL ) )
			{
				...
				fPathManager->PathStatusChanged ( path, ( UInt64 ) arg );
				messageClients ( kIOMessageServicePropertyChange );
...
IOSCSITargetDevice::PerformREPORTLUNS ( void )
{
	...
	// command.
	__Require ( ( fTargetANSIVersion >= kINQUIRY_ANSI_VERSION_SCSI_SPC_Compliant ), ErrorExit );
	...
	__Require_noErr ( header, ErrorExit );
...
ReleaseBuffer:
	...
	__Require_noErr_Quiet ( buffer, ReleaseHeader );
...
ReleaseHeader:
	...
	__Require_noErr_Quiet ( header, ErrorExit );
...
IOSCSITargetDevice::PublishDefaultINQUIRYInformation ( void )
{
	...
	__Require_noErr ( inqData, ErrorExit );
	...
	__Require_noErr ( data, ReleaseBuffer );
...
ReleaseBuffer:
	...
	__Require_noErr_Quiet ( inqData, ErrorExit );
	IODelete ( inqData, SCSICmd_INQUIRY_StandardDataAll, 1 );
	inqData = NULL;
										...
										SCSILogicalUnitNumber	logicalUnit )
{
	...
	result = RetrieveINQUIRYDataPage ( logicalUnit,
									   ...
									   length );
	__Require ( result, ReleaseBuffer );
	__Require ( ( data->PAGE_CODE == kINQUIRY_Page00_PageCode ), ReleaseBuffer );
	__Require ( ( data->PAGE_LENGTH == pageLength ), ReleaseBuffer );
...
ReleaseBuffer:
	...
	__Require_noErr_Quiet ( buffer, ErrorExit );
	buffer->release ( );
	buffer = NULL;
						...
						UInt32									dataSize )
{
	...
	bufferDesc = IOMemoryDescriptor::withAddress ( ( void * ) dataBuffer,
												   dataSize,
												   kIODirectionIn );
	__Require_noErr ( bufferDesc, ErrorExit );
	
 	request = GetSCSITask ( );
 	__Require_noErr ( request, ReleaseBuffer );
...
ReleaseBuffer:
	...
	__Require_noErr_Quiet ( bufferDesc, ErrorExit );
	bufferDesc->release ( );
	bufferDesc = NULL;
						...
						SCSILogicalUnitNumber		logicalUnit )
{
	...
	request = GetSCSITask ( );
	__Require_noErr ( request, ErrorExit );
...
IOSCSITargetDevice::CreateLogicalUnit ( SCSILogicalUnitNumber logicalUnit )
{
	...
	__Require_noErr ( nub, ErrorExit );
	
	result = nub->init ( 0 );
	__Require ( result, ReleaseNub );
	...
	__Require_Action_Quiet ( result, ReleaseNub, nub->detach ( this ) );
	
	
ReleaseNub:
	
	
	__Require_noErr_Quiet ( nub, ErrorExit );
	nub->release ( );
	nub = NULL;
...
ReleaseTask:
	...
	__Require_noErr_Quiet ( request, ReleaseBuffer );
	ReleaseSCSITask ( request );
	request = NULL;
											  ...
											  SCSILogicalUnitNumber	logicalUnit )
{
	...
	__Require_noErr ( data->PAGE_LENGTH, ReleaseBuffer );
	...
	result = RetrieveINQUIRYDataPage ( logicalUnit,
									   ...
									   length );
	__Require ( result, ReleaseBuffer );
	__Require ( ( data->PAGE_CODE == kINQUIRY_Page80_PageCode ), ReleaseBuffer );
	__Require ( ( data->PAGE_LENGTH == pageLength ), ReleaseBuffer );
							...
							SCSILogicalUnitNumber	logicalUnit )
{
	...
	__Require ( result, ReleaseBuffer );
	__Require ( ( data->PAGE_CODE == kINQUIRY_Page83_PageCode ), ReleaseBuffer );
	__Require ( ( pageLength == data->PAGE_LENGTH ), ReleaseBuffer );
	
	// Create the array to hold the ID dictionaries
	deviceIDs = OSArray::withCapacity ( 1 );
	__Require_noErr ( deviceIDs, ReleaseBuffer );
	...
	while ( offset < length )
	{
		...
		__Require_noErr ( idDictionary, ReleaseDeviceIDs );
		...
		__Require_Action ( ( offset <= length ), ReleaseDeviceIDs, idDictionary->release ( ) );
...
ReleaseDeviceIDs:
	...
	__Require_noErr_Quiet ( deviceIDs, ErrorExit );
	deviceIDs->release ( );
	deviceIDs = NULL;
