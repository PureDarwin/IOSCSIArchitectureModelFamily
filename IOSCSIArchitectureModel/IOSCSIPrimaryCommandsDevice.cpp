...
IOSCSIPrimaryCommandsDevice::start ( IOService * provider )
{
	...
	__Require_noErr ( fIOSCSIPrimaryCommandsDeviceReserved, ErrorExit );
	...
	__Require_noErr ( fTaskIDLock, FreeReservedMemory );
	
	fProtocolDriver = OSDynamicCast ( IOSCSIProtocolInterface, provider );
	__Require_noErr ( fProtocolDriver, FreeTaskIDLock );
	
	fDeviceCharacteristicsDictionary = OSDictionary::withCapacity ( 1 );
	__Require_noErr ( fDeviceCharacteristicsDictionary, FreeTaskIDLock );
	...
	// Now create the __Required command sets used by the class
	__Require ( CreateCommandSetObjects ( ), FreeDeviceDictionary );
	...
	__Require ( InitializeDeviceSupport ( ), CloseProvider );
...
IOSCSIPrimaryCommandsDevice::CheckPowerConditionsModePage ( void )
{
    ...
    setProperty ( kIOPropertyPowerConditionsSupporSCSIPrimaryCommandsditions );
...
IOSCSIPrimaryCommandsDevice::TaskCallback ( SCSITaskIdentifier completedTask )
{
	...
	__Require_nonzero ( scsiRequest, ErrorExit );
	
	owner = ( IOSCSIPrimaryCommandsDevice * ) scsiRequest->GetTaskOwner ( );
	__Require_nonzero ( owner, ErrorExit );
					...
					UInt32				timeoutDuration )
{
	...
	__Require ( IsProtocolAccessEnabled ( ), ProtocolAccessDisabledError );
						...
						bool *						use10ByteModeSense )
{
	...
	request = GetSCSITask ( );
	__Require_nonzero ( request, ErrorExit );
