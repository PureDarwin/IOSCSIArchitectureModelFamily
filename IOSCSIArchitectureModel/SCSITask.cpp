...
SCSITask::init ( void )
{
	...
	__Require ( super::init ( ), ErrorExit );
...
SCSITask::ResetForNewTask ( void )
{
	...
	// it has completed.
	__Require ( ( IsTaskActive ( ) == false ), ErrorExit );
...
SCSITask::SetTaskOwner ( OSObject * taskOwner )
{
	...
	__Require_noErr ( taskOwner, ErrorExit );
	taskOwner->retain ( );
...
SCSITask::SetAutoSenseData ( SCSI_Sense_Data * senseData, UInt8 senseDataSize )
{
	...
	else
	{
		...
		__Require_noErr ( fAutosenseDescriptor, Exit );
							 ...
							 UInt8				senseDataSize )
{
	...
	__Require ( fAutoSenseDataIsValid, Exit );
	__Require_noErr_Action ( receivingBuffer, Exit, result = fAutoSenseDataIsValid );
...
SCSITask::EnsureAutosenseDescriptorExists ( void )
{
	...
	if ( fAutosenseDescriptor == NULL )
	{
		...
		//fAutosenseDescriptor = IOMemoryDescriptor::withAddressRange(vm_address_t addr64_t, mach_vm_size_t long, IOOptionBits options, task_t task)
...
SCSITask::GetProtocolLayerReference ( void )
{
	...
	__Check ( fProtocolLayerReference );
...
SCSITask::GetApplicationLayerReference ( void )
{
	...
	__Check ( fApplicationLayerReference );
