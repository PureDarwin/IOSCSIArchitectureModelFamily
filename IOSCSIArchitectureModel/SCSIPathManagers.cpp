...
SCSIPathSet::withCapacity ( unsigned int capacity )
{
	...
	__Require_noErr ( set, ErrorExit );
	
	result = set->initWithCapacity ( capacity );
	__Require ( result, ReleaseSet );
...
ReleaseSet:
	...
	__Require_noErr_Quiet ( set, ErrorExit );
	set->release ( );
	set = NULL;
...
SCSIPathSet::setObject ( const SCSITargetDevicePath * path )
{
	...
	result = member ( path );
	__Require ( ( result == false ), ErrorExit );
								  ...
								  IOSCSIProtocolServices * 	initialPath )
{
	...
	__Require_noErr ( manager, ErrorExit );
	
	result = manager->InitializePathManagerForTarget ( target, initialPath );
	__Require ( result, ReleasePathManager );
...
ReleasePathManager:
	...
	__Require_noErr_Quiet ( manager, ErrorExit );
	manager->release ( );
	manager = NULL;
...
ReleaseInactivePathSet:
	...
	__Require_noErr_Quiet ( fInactivePathSet, ReleasePathSet );
	fInactivePathSet->release ( );
	fInactivePathSet = NULL;
...
ReleasePathSet:
	...
	__Require_noErr_Quiet ( fPathSet, FreeLock );
	fPathSet->release ( );
	fPathSet = NULL;
...
FreeLock:
	...
	__Require_noErr_Quiet ( fLock, ErrorExit );
	IOLockFree ( fLock );
	fLock = NULL;
...
SCSIPressurePathManager::AddPath ( IOSCSIProtocolServices * interface )
{
	...
	__Require_noErr ( interface, ErrorExit );
	
	path = SCSITargetDevicePath::Create ( this, interface );
	__Require_noErr ( path, ErrorExit );
