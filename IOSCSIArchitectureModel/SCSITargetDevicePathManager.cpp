							   ...
							   IOSCSIProtocolServices * 		initialPath )
{
	...
	__Require ( path->InitWithObjects ( manager, initialPath ), ReleasePath );
...
ReleasePath:
	...
	__Require_noErr_Quiet ( path, ErrorExit );
	path->release ( );
	path = NULL;
						...
						IOSCSIProtocolServices * 		interface )
{
	...
	__Require_noErr ( fStatistics, ErrorExit );
	
	fDomainIdentifier = GetInterfaceDomainIdentifier ( interface );
	__Require_noErr ( fDomainIdentifier, ReleaseStatistics );
	...
	__Require_noErr ( number, ReleaseStatistics );
	...
	__Require_noErr ( string, ReleaseStatistics );
	fPathStatus = string;
...
ReleaseStatistics:
	...
	__Require_noErr_Quiet ( fStatistics, ErrorExit );
	fStatistics->release ( );
	fStatistics = NULL;
...
ReleaseStats:
	...
	__Require_noErr_Quiet ( fStatistics, ReleaseStats );
	fStatistics->release ( );
	fStatistics = NULL;
